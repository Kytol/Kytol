...
/* Pass-through fetch so Chromium treats this SW as installable (PWA). */
...
self.addEventListener('message', (event) => {
  ...
  if (event.data && event.data.type === 'zzz-open-spa') {
    const target = new URL(event.data.url || '/', self.location.origin).href;
    event.waitUntil(
      self.clients.matchAll({ type: 'window', includeUncontrolled: true }).then((list) => {
        for (const client of list) {
          // Prefer an already-open app window when Chrome reports standalone clients first.
        ...
        // On Chromium, openWindow often routes into the installed PWA window.
